 ...
 */
export function computeSurprise(expected: string, actual: string): number {
  const e = normalize(expected);
  const a = normalize(actual);

  if (!e) return 0;
  if (e === a) return 0;
  if (a.includes(e) || e.includes(a)) return 0.2;
  ...
  const te = tokenSet(e);
  const ta = tokenSet(a);
  if (te.size === 0 && ta.size === 0) return 0;

  let inter = 0;
  for (const t of te) if (ta.has(t)) inter++;
