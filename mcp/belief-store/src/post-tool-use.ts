...
const SURPRISE_THRESHOLD = Number(
  process.env.PM_SURPRISE_THRESHOLD ?? '0.6'
);
...
function toObservationText(response: unknown): string {
...
async function main(): Promise<void> {
  ...
  const byHash = db
    .prepare(
      `SELECT action_id, expected, action
         FROM predictions
        WHERE tool_name = ?
          AND tool_input_hash = ?
          AND actual IS NULL
    ...
    .get(toolName, hash) as OpenPrediction | undefined;
  ...
  const byTool = byHash
    ...
    : (db
        ...
        .get(toolName) as OpenPrediction | undefined);
  ...
  db.prepare(
    `UPDATE predictions
      ...
      WHERE action_id = ?`
  ).run(actual, surprise, match.action_id);
  ...
  if (surprise >= SURPRISE_THRESHOLD) {
    const notice = {
      hookSpecificOutput: {
        ...
        additionalContext:
          `[predictive-mind] High prediction error for ${toolName} ` +
